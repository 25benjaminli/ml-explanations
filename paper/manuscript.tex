...
The key to creating and leveraging the network is linear algebra and calculus. Looking at a specific example, we are given 10 soccer players and two pieces of info (\textbf{features}) about their goals per game and pass accuracy. We want to determine if they're going to be drafted to a soccer team. More formally, we are given a batched input $x$ of dimensions $(b, f)$, where $b$ is the number of entries and $f$ is the number of features - say $(10, 2)$. We are trying to predict a vector $y$ of dimensions $(10, 1)$ where 1 indicates that a player is drafted and 0 indicates they are not drafted. 
...
where $A_i$ is the \enquote{activated} output of the layer, $W_i$ is a weight matrix, $B_i$ is the bias matrix, and $\sigma$ is some activation function (e.g. sigmoid, relu) that introduces non-linearity. The intuition is that the weight matrices will automatically learn how to extract meaningful features from the data. However, we'll also need the \textbf{unactivated} output later for backpropagation: $Z_i = W_i * A_{i-1} + B_i$. We randomly initialize weight matrices with dimensions $(n_{i}, n_{i-1})$ and bias matrices of dimensions $(n_{i}, b)$. To understand why these dimensions are used, consider what happens after a pass through the \textbf{first} hidden layer of the network. We treat the original input X as the precursor activated value since there doesn't exist at this point. 
...
When we say CNNs reduce the spatial dimensions of the data in exchange for increased semantic depth, what exactly does that mean? Each convolution layer consists of many kernels, or filters, each of which is a matrix. Consider this $5 \times 5$ grayscale matrix representing a \enquote{0} shape (values closer to 1 are more solid).  
...
Can you see this? The human eye and brain are very good at recognizing patterns - we can immediately see that the values close to 1 form a 0 shape. But how is a computer supposed to grasp that concept? The key is applying \textbf{kernels}. 
...
As indicated in Fig. \ref{fig:convolutions}, these kernels are applied to data by taking the hadamard (element-wise) product of the original matrix with the filter followed by the sum. Then, the kernel "slides over" the entire image by a specified stride length, and the convolution operation is repeated until the entire input has been processed. Let's determine what this kernel does:
\[
\begin{bmatrix}
1 & 1 & 1 \\
0 & 0 & 0 \\
...
Where $pos$ represents the index of the token in the sequence, $i$ is the position in the embedding vector, $d$ is the total embedding dimension. 

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{../media/sinusoidal.png}
  \caption{Sinusoidal embeddings. \href{https://learnopencv.com/sinusoidal-position-embeddings/}{Credit for this illustration}}
  \label{fig:sinusoidal}
\end{figure}

Above is a visualization of sinusoidal embeddings for a sentence. Each line traces the value of a selected dimension in the embedding vector of a particular word (only 8 dimensions are shown here but in practice there are a lot more) as you progress through the sentence.  
...
\textbf{Self-attention}: we begin by creating three learnable weight matrices: $W_Q$, $W_K$, $W_V$, each dimensions $d_{model}$ for query, key, value respectively. Then, right-multiply $X$ by each of these matrices, creating three resulting matrices of the same dimensions $Q$, $K$, $V$. To understand what these matrices represent, I found it helpful to frame matrix multiplication as the dot product between every row in $X$ (i.e. token) and every column in $W$. Recall that the dot product between two vectors encodes similarity (i.e. high value = similar, low value = dissimilar). With this in mind, look at the query matrix $W_q$ and consider each column of $W_q$ as encoding some \enquote{search term}. Select an arbitrary row in the matrix $X$, say the first row. 
...
It's desirable to quantify uncertainty in a model's predictions especially in high-stakes environments (e.g. healthcare diagnostics, autonomous driving, financial forecasting). Uncertainty can be classified into two categories:
\begin{enumerate}
  \item \textbf{Aleatoric}: arising from inherent randomness in systems and cannot be reduced.
  \item \textbf{Epistemic}: arising from data limitations and/or ill-defined model and can be reduced. 
\end{enumerate}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.6\textwidth]{../media/aleatoric_epistemic.png}
  \caption{Aleatoric vs. Epistemic Uncertainty. \href{https://commons.wikimedia.org/wiki/File:An-illustration-of-the-difference-between-aleatoric-and-epistemic-uncertainties-The-dots.png}{Credit for this illustration}}
  \label{fig:aleatoric_epistemic}
\end{figure}

Thus, it's not enough to have simply one estimate of "uncertainty". Differentiating between these two forms of uncertainty is important for decision-making and also for developers to know how to improve their approaches. There exist various UQ approaches that each take unique approaches to capturing uncertainty.  

\subsection{Bayesian}

Traditional neural networks have point estimates for model weights. Bayesian approaches attempt to assign a distribution $p(\theta \mid D)$ where $D$ is the dataset - this is the \textbf{posterior} (can be computed via methods such as variational inference), modeled by:

\[
p(\theta \mid D) = \frac{p(D \mid \theta) \cdot p(\theta)}{p(D)}
\]

Interpret $p(\theta \mid D)$ the probability density function of parameters over the dataset $D$, thus whenever you sample from it you get a new set of parameters $\theta_s$. Turns out computing the exact posterior is computationally intractable due to the \textbf{marginal likelihood} $$P(D) = \int p(D | \theta) p(\theta) \ d\theta$$

Here, integrating the likelihood $p(D \mid \theta)$ over all possible $\theta$ is very difficult because it needs a full pass through the dataset (computing $\prod p(y_i \mid x_i)$ through standard forward pass) for each possible value of $\theta$. To avoid computing the entire posterior we can approximate it via variational inference - use simpler $q(\theta \mid \phi)$ and minimize the KL divergence with respect to the original intractable marginal likelihood. 

$$KL(q(\theta \mid \phi) \mid \mid P(\theta \mid D)) = \int q(\theta \mid \phi) \cdot \log{\frac{q(\theta \mid \phi)}{P(\theta \mid D)}}$$

Because this still contains the intractable $P(\theta \mid D)$, we can use a trick and end up maximizing this other objective ELBO (Evidence Lower Bound). $ELBO(\phi) = \mathbb{E}_q [\log P(D \mid \theta)] - KL(q(\theta \mid \phi) \mid \mid P(\theta))$. Intuitively, the first part is asking for the expected value under distribution $q$. Sample a bunch of $\theta$ from $q$, calculate the log likelihood for each, and take the average of all those values. Parameters that better fit the data will increase the value of this term. The second part (KL divergence between q distribution and prior) prevents the new estimate from straying too far away from the prior. 

Notably, variational inference requires storing double the parameters (mean AND variance) since those are optimized via:

\begin{align*}
ELBO(\phi) &= \mathbb{E}_q [\log P(D \mid \theta)] - KL(q(\theta \mid \phi) \mid \mid P(\theta))
\\ 
\mu &= \mu + lr * \nabla_\mu ELBO
\\
\sigma &= \sigma + lr * \nabla_\sigma ELBO
...
Once the posterior is approximated, sample weights and run the network over the data to produce different outputs. The outputs form another distribution, from which you can conduct analysis such as constructing "credible" intervals. 

Another strategy for bayesian-like estimations is using dropout during training, keeping it on during inference, and constructing a distribution of possible results. But this is not a bayesian neural net in the traditional sense. 

Bayesian neural nets have a few advantages and disadvantages. They works well with smaller datasets due to preventing significant drift from the prior. Secondly, they model epistemic uncertainty by calculating metrics based off different model parameters (and can be modified to capture aleatoric uncertainty as well). However, they are still slow... especially during inference and because of two extra parameters to optimize. 

\subsection{Conformal Prediction}

Conformal prediction enables distribution-free estimation of uncertainty via calibrating models to produce outcomes with certain coverage guarantees. For instance, a neural network might have three outputs: 2.5th quantile (i.e. it thinks the ground truth will fall below this value 2.5 percent of the time), 50th quantile ("median" image), and 97.5th quantile (ground truth will fall below 97.5 percent of the time). This model might be trained with something like pinball loss:
...
where $\tau$ is the desired quantile. This loss penalizes over and underestimation depending on the quantile and can be visualized as such:

\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{../media/pinball.png}
  \caption{Pinball loss. Observe that for higher quantiles, overprediction (negative prediction error) leads to higher loss and this is opposite for lower quantiles. }
  \label{fig:pinball}
\end{figure}

However, it's not sufficient to just train a model with this loss. Because it's a stochastic process, there are no statistical guarantees. This means we hold out a calibration set (separate from train and test sets). The calibration set is used to scale and refine the prediction intervals such that the average error over the data remains below a permitted threshold. You might, for instance, scale the upper and lower bounds such that they guarantee that a prediction is contained between them 95\% of the time. 

\subsection{Resources}

\begin{itemize}
  \item \href{https://arxiv.org/pdf/2402.19460}{Benchmarking Uncertainty Disentanglement: Specialized Uncertainties for Specialized Tasks}
  \item \href{https://www.cs.toronto.edu/~duvenaud/distill_bayes_net/public/}{Bayesian NN deep dive}
  \item \href{https://towardsdatascience.com/conformal-prediction-for-machine-learning-classification-from-the-ground-up-a12fcf6860d0/}{Conformal prediction hands-on code}
\end{itemize}
...
Now, we have a trained reward model. Freeze the weights of the original LM ($\pi_{ref}$) make a trainable copy of the original LM and call it $\pi_{\phi}$ (this will be the final fine-tuned LM after the entire process is complete). 

To prevent $\pi_{\phi}$ from \enquote{reward hacking} (i.e. giving nonsensical outputs to fool the reward model), calculate KL divergence between outputs of $\pi_{\phi}$ and $\pi_{ref}$. This is given as $KL = \beta\logp{\frac{\pi_{\phi}(y|x)}{\pi_{ref}(y|x)}}$. The further $\pi_{\phi}(y|x)$ is from $\pi_{ref}(y|x)$, the greater the KL divergence. 
