...
\begin{figure}[H]
  ...
  \caption{Aleatoric vs. Epistemic Uncertainty}
...
Interpret $p(\theta \mid D)$ the probability density function of parameters over the dataset $D$, thus whenever you sample from it you get a new set of parameters $\theta_s$. Turns out computing the exact posterior is computationally intractable due to the \textbf{marginal likelihood} $$\int p(D | w) p(w) \ dw$$
